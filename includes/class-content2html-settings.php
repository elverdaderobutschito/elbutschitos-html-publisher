...
class Content2HTML_Settings {
    ...
    public function registerMenu(): void {
        add_menu_page(
            __('Elbutschitos HTML Publisher', 'elbutschitos-html-publisher'),
    ...
    public function enqueueAssets(string $hook): void {
        ...
        wp_localize_script('content2html-deploy-admin', 'content2htmlDeploy', [
            ...
            'batchSize' => 5,
            'i18n' => [
                'testConnection' => __('Test connection', 'elbutschitos-html-publisher'),
                'deployingToTarget' => __('Deploying to target …', 'elbutschitos-html-publisher'),
                'deployAll' => __('Deploy all', 'elbutschitos-html-publisher'),
                'running' => __('Running …', 'elbutschitos-html-publisher'),
                'deployingAssets' => __('Deploying assets …', 'elbutschitos-html-publisher'),
 		     ...
 		     . __('"%s" uses the last SAVED settings, not your current input.', 'elbutschitos-html-publisher') . "\n\n"
                ...
                'errorPrefix' => __('Error:', 'elbutschitos-html-publisher'),
                'errorRequestFailed' => __('Error processing the request.', 'elbutschitos-html-publisher'),
                'starting' => __('Starting …', 'elbutschitos-html-publisher'),
                'noMatchingPosts' => __('No matching posts/pages found.', 'elbutschitos-html-publisher'),
                'errorStartingDeploy' => __('Error starting the deployment.', 'elbutschitos-html-publisher'),
                'errorBatchProcessing' => __('Error during batch processing.', 'elbutschitos-html-publisher'),
                'errorFinalizing' => __('Error during final upload.', 'elbutschitos-html-publisher'),
                'assetsDeployedWithUrl' => __('Assets deployed. Deploy:', 'elbutschitos-html-publisher'),
                'assetsDeployed' => __('Assets deployed.', 'elbutschitos-html-publisher'),
    ...
    public function handleSave(): void {
        if (!current_user_can('manage_options')) {
            wp_die(esc_html__('Insufficient permissions.', 'elbutschitos-html-publisher'));
        }
        ...
        if (!empty($_FILES['template_file']['tmp_name'])) {
            ...
            if (isset($uploaded['file'])) {
                ...
                set_transient('content2html_template_upload_error', $uploaded['error'] ?? __('Unknown error while uploading the default template.', 'elbutschitos-html-publisher'), MINUTE_IN_SECONDS * 5);
            }
        ...
        if (!empty($_POST['new_template_name'])) {
            if (!empty($_FILES['new_template_file']['tmp_name'])) {
                ...
                if (isset($uploadedTemplate['file'])) {
                    ...
                    set_transient('content2html_template_upload_error', $uploadedTemplate['error'] ?? __('Unknown error while uploading.', 'elbutschitos-html-publisher'), MINUTE_IN_SECONDS * 5);
                }
                ...
                set_transient('content2html_template_upload_error', __('A name for the new template was given, but no file was selected.', 'elbutschitos-html-publisher'), MINUTE_IN_SECONDS * 5);
            }
            ...
            set_transient('content2html_template_upload_error', __('Please enter a name for the new template (required) - the file was therefore not saved.', 'elbutschitos-html-publisher'), MINUTE_IN_SECONDS * 5);
        }
        ...
        if ($newValues['forms_enabled'] === 'on' && $newValues['target'] === 'sftp' && $newValues['form_recipient_email'] === '') {
            set_transient(
                ...
                __('Forms are enabled, but no recipient email address has been set - the form handler will reject incoming submissions until this is fixed.', 'elbutschitos-html-publisher'),
                MINUTE_IN_SECONDS * 5
            );
        }
        ...
        if (isset($_FILES['assets_zip']['error']) && $_FILES['assets_zip']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            if ($error !== UPLOAD_ERR_OK) {
                ...
                $assetsResult = isset($_FILES['assets_zip']['tmp_name'])
                    ...
                    : ['ok' => false, 'message' => __('Unknown error while uploading.', 'elbutschitos-html-publisher'), 'warnings' => []];
            }
            ...
            set_transient('content2html_assets_upload_result', [
                ...
                'message' => sprintf(
                    ...
                    __('The request exceeds the server limit post_max_size (currently: %s). The file is too large to upload in this form.', 'elbutschitos-html-publisher'),
    ...
    public function handleMarkdownExport(): void {
        ...
        try {
            ...
            wp_die(esc_html__('Export failed:', 'elbutschitos-html-publisher') . ' ' . esc_html($e->getMessage()));
        }

        if (!is_file($zipPath)) {
            wp_die(esc_html__('Export failed: the ZIP file was not created.', 'elbutschitos-html-publisher'));
        }
    ...
    private function renderNavigationFields(string $prefix, string $label, array $settings): void {
        ...
        <table class="form-table content2html-nav-fields" data-prefix="<?php echo esc_attr($prefix); ?>">
            <tr>
                <th><?php esc_html_e('Generate automatically', 'elbutschitos-html-publisher'); ?></th>
                <td>
                    <label>
                        ...
                        <?php esc_html_e('Replace navigation markers in the template with real menu items', 'elbutschitos-html-publisher'); ?>
                    </label>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>menu_id"><?php esc_html_e('WordPress menu', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    <select id="<?php echo esc_attr($prefix); ?>menu_id" name="<?php echo esc_attr($prefix); ?>menu_id">
                        <option value="0">– <?php esc_html_e('Select a menu', 'elbutschitos-html-publisher'); ?> –</option>
                    ...
                    <?php if (empty($menus)): ?>
                        <p class="description"><?php esc_html_e('No menus found - create one first under Appearance → Menus.', 'elbutschitos-html-publisher'); ?></p>
                    <?php endif; ?>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>wrapper_marker"><?php esc_html_e('Wrapper marker', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Wraps the ENTIRE menu area in the template (e.g. the outer <ul>).', 'elbutschitos-html-publisher'); ?></p>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>item_marker"><?php esc_html_e('Item marker', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Wraps EXACTLY ONE demo menu item without sub-items (e.g. one <li>) - duplicated for each real menu item.', 'elbutschitos-html-publisher'); ?></p>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>parent_item_marker"><?php esc_html_e('Parent item marker (optional)', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Only needed for multi-level menus. Without this marker, sub-items are ignored (flat display).', 'elbutschitos-html-publisher'); ?></p>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>submenu_wrapper_marker"><?php esc_html_e('Submenu wrapper marker', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Only relevant if "Parent item marker" is set - wraps the container for sub-items INSIDE the parent template.', 'elbutschitos-html-publisher'); ?></p>
                </td>
            </tr>
            <tr class="content2html-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>submenu_item_marker"><?php esc_html_e('Submenu item marker (optional)', 'elbutschitos-html-publisher'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Leave empty to use the same template as "Item marker" for sub-items.', 'elbutschitos-html-publisher'); ?></p>
                </td>
            </tr>
        </table>
        <?php
        ...
        switch ($error) {
            case UPLOAD_ERR_INI_SIZE:
                return sprintf(
                    ...
                    __('The file exceeds the server limit upload_max_filesize (currently: %s). Please shrink the ZIP or ask your host to raise the limit.', 'elbutschitos-html-publisher'),
                    ini_get('upload_max_filesize')
                );
            case UPLOAD_ERR_FORM_SIZE:
                return __('The file exceeds the maximum allowed by the form.', 'elbutschitos-html-publisher');
            case UPLOAD_ERR_PARTIAL:
                return __('The file was only partially uploaded. Please try again.', 'elbutschitos-html-publisher');
            case UPLOAD_ERR_NO_TMP_DIR:
                return __('Server error: no temporary directory available for uploads.', 'elbutschitos-html-publisher');
            case UPLOAD_ERR_CANT_WRITE:
                return __('Server error: the file could not be written to disk.', 'elbutschitos-html-publisher');
            case UPLOAD_ERR_EXTENSION:
                return __('The upload was stopped by a PHP extension.', 'elbutschitos-html-publisher');
            default:
                return sprintf(
                    /* translators: %d: PHP upload error code */
                    __('Unknown upload error (code %d).', 'elbutschitos-html-publisher'),
                    $error
                );
        }
    }
    ...
    public function renderPage(): void {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Elbutschitos HTML Publisher – Settings', 'elbutschitos-html-publisher'); ?></h1>

            <div id="content2html-unsaved-notice" class="notice notice-warning" style="display:none;">
                <p>
                    <strong><?php esc_html_e('You have unsaved changes.', 'elbutschitos-html-publisher'); ?></strong>
                    <?php esc_html_e('"Deploy all" and "Deploy assets only" use the last', 'elbutschitos-html-publisher'); ?>
                    <strong><?php esc_html_e('saved', 'elbutschitos-html-publisher'); ?></strong> <?php esc_html_e('settings, not your current input - please click "Save settings" first.', 'elbutschitos-html-publisher'); ?>
                </p>
            </div>
            ...
            if (isset($_GET['saved'])): ?>
                <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Settings saved.', 'elbutschitos-html-publisher'); ?></p></div>
            <?php endif; ?>
            ...
            if ($assetsUploadResult) {
                ...
                <div class="notice <?php echo esc_attr($noticeClass); ?> is-dismissible">
                    ...
                    <?php if (!empty($assetsUploadResult['warnings'])): ?>
                        <p><strong><?php esc_html_e('Note: only CSS, JS, font and image files are kept - anything else found in the ZIP was removed', 'elbutschitos-html-publisher'); ?></strong>:</p>
            ...
            <?php if (!Content2HTML_Crypto::usesConfigKey()): ?>
                <div class="notice notice-warning">
                    <p>
                        <?php esc_html_e('No', 'elbutschitos-html-publisher'); ?> <code>CONTENT2HTML_ENCRYPTION_KEY</code> <?php esc_html_e('found in', 'elbutschitos-html-publisher'); ?> <code>wp-config.php</code>.
                        <?php esc_html_e('Secrets are still stored encrypted, but with an auto-generated key that also lives in the database. Recommended:', 'elbutschitos-html-publisher'); ?>
                        <code>define('CONTENT2HTML_ENCRYPTION_KEY', '<?php esc_html_e('a-long-random-value', 'elbutschitos-html-publisher'); ?>');</code>
                        <?php esc_html_e('add to wp-config.php.', 'elbutschitos-html-publisher'); ?>
                    </p>
                </div>
            <?php endif; ?>
            ...
            <?php if ($settings['target'] === 'sftp' && !$hasVendor): ?>
                <div class="notice notice-error">
                    <p>
                        <?php esc_html_e('SFTP target selected, but', 'elbutschitos-html-publisher'); ?> <code>vendor/phpseclib</code> <?php esc_html_e('was not found. See README.md in the plugin directory for installation.', 'elbutschitos-html-publisher'); ?>
                    </p>
                </div>
            <?php endif; ?>
            ...
            <form id="content2html-settings-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
                ...
                <h2 class="nav-tab-wrapper content2html-tabs">
                    <a href="#" class="nav-tab" data-tab="inhalte"><?php esc_html_e('Content', 'elbutschitos-html-publisher'); ?></a>
                    <a href="#" class="nav-tab" data-tab="navigation"><?php esc_html_e('Navigation', 'elbutschitos-html-publisher'); ?></a>
                    <a href="#" class="nav-tab" data-tab="formulare"><?php esc_html_e('Forms', 'elbutschitos-html-publisher'); ?></a>
                    <a href="#" class="nav-tab" data-tab="ziel"><?php esc_html_e('Deployment target', 'elbutschitos-html-publisher'); ?></a>
                    <a href="#" class="nav-tab" data-tab="markdown"><?php esc_html_e('Markdown export', 'elbutschitos-html-publisher'); ?></a>
                </h2>

                <div class="content2html-tab-panel" data-tab-panel="inhalte">
                <h2 class="title"><?php esc_html_e('Content', 'elbutschitos-html-publisher'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Post Types', 'elbutschitos-html-publisher'); ?></th>
                        <td>
                            <label><input type="checkbox" name="post_types[]" value="post" <?php checked(in_array('post', $settings['post_types'], true)); ?>> <?php esc_html_e('Posts', 'elbutschitos-html-publisher'); ?></label><br>
                            <label><input type="checkbox" name="post_types[]" value="page" <?php checked(in_array('page', $settings['post_types'], true)); ?>> <?php esc_html_e('Pages', 'elbutschitos-html-publisher'); ?></label>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="content2html_template"><?php esc_html_e('Template file', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            <?php if (!empty($settings['template_path'])): ?>
                                <p><?php esc_html_e('Current:', 'elbutschitos-html-publisher'); ?> <code><?php echo esc_html(basename($settings['template_path'])); ?></code></p>
                            ...
                            <p class="description"><?php esc_html_e('HTML file with placeholders (e.g.', 'elbutschitos-html-publisher'); ?> <code>###title###</code>). <?php esc_html_e('Leave empty to keep the current file.', 'elbutschitos-html-publisher'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Additional templates', 'elbutschitos-html-publisher'); ?></th>
                        <td>
                            <?php if (!empty($settings['extra_templates'])): ?>
                                <table class="widefat" style="max-width: 500px; margin-bottom: 10px;">
                                    <thead><tr><th><?php esc_html_e('Name', 'elbutschitos-html-publisher'); ?></th><th><?php esc_html_e('File', 'elbutschitos-html-publisher'); ?></th><th><?php esc_html_e('Remove', 'elbutschitos-html-publisher'); ?></th></tr></thead>
                                    <tbody>
                            ...
                            <?php else: ?>
                                <p class="description"><?php esc_html_e('No additional templates set up yet.', 'elbutschitos-html-publisher'); ?></p>
                            <?php endif; ?>

                            <p>
                                <label for="new_template_name"><?php esc_html_e('Add new template (name required):', 'elbutschitos-html-publisher'); ?></label><br>
                                <input type="text" id="new_template_name" name="new_template_name" class="regular-text" placeholder="<?php echo esc_attr__('e.g. Landing page (required so it appears in the dropdown)', 'elbutschitos-html-publisher'); ?>">
                                <input type="file" id="new_template_file" name="new_template_file" accept=".html,.htm">
                            </p>
                            <p class="description">
                                <?php esc_html_e('Then available per post/page as an alternative to the default template (dropdown in the "HTML Publisher" meta box on the edit screen).', 'elbutschitos-html-publisher'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($assetsStatus['exists']): ?>
                                <p>
                                    <?php esc_html_e('Currently set up:', 'elbutschitos-html-publisher'); ?> <strong><?php echo esc_html((string) $assetsStatus['file_count']); ?> <?php esc_html_e('files', 'elbutschitos-html-publisher'); ?></strong>
                                    (<?php echo esc_html(size_format($assetsStatus['total_size'])); ?>)
                                    <?php if (!empty($assetsStatus['updated_at'])): ?>
                                        &ndash; <?php esc_html_e('last updated on', 'elbutschitos-html-publisher'); ?> <?php echo esc_html(date_i18n('d.m.Y H:i', $assetsStatus['updated_at'])); ?>
                                    <?php endif; ?>
                                </p>
                            <?php else: ?>
                                <p><?php esc_html_e('No assets set up yet.', 'elbutschitos-html-publisher'); ?></p>
                            <?php endif; ?>
                            <input type="file" id="content2html_assets" name="assets_zip" accept=".zip">
                            <p class="description">
                                <?php esc_html_e('ZIP file whose root already is a folder named', 'elbutschitos-html-publisher'); ?> <code>assets/</code>
                                (<?php esc_html_e('i.e. zip the assets folder itself, not just its contents', 'elbutschitos-html-publisher'); ?>).
                                <?php esc_html_e('Copied 1:1 into the build folder so template references like', 'elbutschitos-html-publisher'); ?>
                                <code>href="assets/css/styles.css"</code> <?php esc_html_e('work.', 'elbutschitos-html-publisher'); ?>
                            </p>
                            <p class="description">
                                <?php esc_html_e('Server upload limit: max.', 'elbutschitos-html-publisher'); ?> <code><?php echo esc_html(ini_get('upload_max_filesize')); ?></code>
                                <?php esc_html_e('per file,', 'elbutschitos-html-publisher'); ?> <code><?php echo esc_html(ini_get('post_max_size')); ?></code> <?php esc_html_e('per form submission overall. If your ZIP is bigger, please compress/trim assets first (e.g. remove unused font weights) or ask your host to raise the limit.', 'elbutschitos-html-publisher'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="date_format"><?php esc_html_e('Date format', 'elbutschitos-html-publisher'); ?></label></th>
			...
			<th><label for="remove_class_prefixes"><?php esc_html_e('Remove CSS class prefixes', 'elbutschitos-html-publisher'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="tidy_html_rules"><?php esc_html_e('Tidy HTML rules', 'elbutschitos-html-publisher'); ?></label></th>
		...
		<div id="content2html-field-browser-modal" class="content2html-modal-overlay" style="display:none;">
                ...
                <h2 class="title"><?php esc_html_e('Forms', 'elbutschitos-html-publisher'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Enable forms', 'elbutschitos-html-publisher'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="forms_enabled" <?php checked('on', $settings['forms_enabled']); ?>>
                                <?php esc_html_e('Make &lt;form&gt; tags on generated pages functional', 'elbutschitos-html-publisher'); ?>
                            </label>
                            <p class="description">
                                <strong>Netlify:</strong> <?php esc_html_e('forms automatically get', 'elbutschitos-html-publisher'); ?> <code>data-netlify="true"</code>
                                <?php esc_html_e('and a hidden', 'elbutschitos-html-publisher'); ?> <code>form-name</code> <?php esc_html_e('field - Netlify Forms then detects and processes them on its own, no server code needed.', 'elbutschitos-html-publisher'); ?><br>
                                <strong>SFTP:</strong> <?php esc_html_e('the form', 'elbutschitos-html-publisher'); ?> <code>action</code> <?php esc_html_e('is rewritten to a bundled PHP handler', 'elbutschitos-html-publisher'); ?> (<code>form-handler.php</code>) <?php esc_html_e('that emails submissions. Requires the target server to run PHP.', 'elbutschitos-html-publisher'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_recipient_email"><?php esc_html_e('Recipient email (SFTP only)', 'elbutschitos-html-publisher'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="form_from_email"><?php esc_html_e('Sender address (SFTP only)', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Leave empty to use the server default address. Many hosts only accept sender addresses from their own domain.', 'elbutschitos-html-publisher'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_subject_prefix"><?php esc_html_e('Subject prefix (SFTP only)', 'elbutschitos-html-publisher'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="form_redirect_url"><?php esc_html_e('Thank-you page (Netlify + SFTP)', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('Leave empty to redirect back to the originating page after submission automatically. Important for Netlify: the form\'s original', 'elbutschitos-html-publisher'); ?> <code>action</code> (<?php esc_html_e('points to a WordPress URL', 'elbutschitos-html-publisher'); ?>) <?php esc_html_e('is always replaced - either by this thank-you page, or, if empty, by removing the', 'elbutschitos-html-publisher'); ?> <code>action</code> (<?php esc_html_e('submits to the current page', 'elbutschitos-html-publisher'); ?>).
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_honeypot_field"><?php esc_html_e('Honeypot field name (SFTP only)', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Simple spam deterrent: a CSS-hidden field that only bots fill in. Letters/numbers/underscore/hyphen only.', 'elbutschitos-html-publisher'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_custom_action"><?php esc_html_e('Custom form target (SFTP only)', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('Leave empty to use our bundled PHP handler', 'elbutschitos-html-publisher'); ?> (<code>/form-handler.php</code>) - <?php esc_html_e('requires the target server to run PHP. If something else runs there (e.g. your own endpoint, an external form service), enter its URL here - the form', 'elbutschitos-html-publisher'); ?> <code>action</code> <?php esc_html_e('is then rewritten to it, and our', 'elbutschitos-html-publisher'); ?> <code>form-handler.php</code> <?php esc_html_e('is not created/uploaded in that case.', 'elbutschitos-html-publisher'); ?>
                            </p>
                        </td>
                    </tr>
                </table>

                <h3><?php esc_html_e('Client-side validation', 'elbutschitos-html-publisher'); ?></h3>
                <p class="description">
                    <?php esc_html_e('Runs automatically as soon as forms are enabled above (for Netlify AND SFTP) - no extra toggle needed. Checks in the browser BEFORE submitting: fields with', 'elbutschitos-html-publisher'); ?>
                    <code>aria-required="true"</code> <?php esc_html_e('must be filled in (for checkbox/radio groups: at least one option),', 'elbutschitos-html-publisher'); ?>
                    <code>type="email"</code> <?php esc_html_e('fields must look like an email address. Most accessible form plugins (Fluent Forms, WPForms, Gravity Forms etc.) already set these attributes automatically - no configuration needed. If your form plugin doesn\'t set these attributes, the check simply doesn\'t apply (data is then submitted unvalidated, as before).', 'elbutschitos-html-publisher'); ?>
                </p>
                </div>

                <div class="content2html-tab-panel" data-tab-panel="navigation">
                <h2 class="title"><?php esc_html_e('Navigation', 'elbutschitos-html-publisher'); ?></h2>
                <p class="description">
                    <?php esc_html_e('Replaces marker comments in the template with real menu items from a WordPress menu. Details for each marker are right next to the respective fields below.', 'elbutschitos-html-publisher'); ?>
                </p>
                <?php $this->renderNavigationFields('nav_main_', __('Main navigation', 'elbutschitos-html-publisher'), $settings); ?>
                <?php $this->renderNavigationFields('nav_footer_', __('Footer navigation', 'elbutschitos-html-publisher'), $settings); ?>
                <table class="form-table">
                    <tr>
                        <th><label for="nav_active_class"><?php esc_html_e('CSS class for active page', 'elbutschitos-html-publisher'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Set on the first &lt;a&gt; tag of the menu item matching the page currently being generated - applies to main and footer navigation alike.', 'elbutschitos-html-publisher'); ?></p>
                        </td>
                    </tr>
                </table>
                </div>
                ...
                <h2 class="title"><?php esc_html_e('Deployment target', 'elbutschitos-html-publisher'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Target', 'elbutschitos-html-publisher'); ?></th>
                        <td>
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="description"><?php echo !empty($settings['netlify_token']) ? esc_html__('A token is already saved. Leave empty to keep it.', 'elbutschitos-html-publisher') : esc_html__('No token saved yet.', 'elbutschitos-html-publisher'); ?></p>
                            </td>
                        </tr>
                    </table>
                    <p>
                        <button type="button" class="button" id="content2html-test-netlify-btn"><?php esc_html_e('Test connection', 'elbutschitos-html-publisher'); ?></button>
                        <span id="content2html-test-netlify-result" class="content2html-test-result"></span>
                    </p>
                    <p class="description">
                        <?php esc_html_e('Important: Netlify deploys always replace the entire site content. The single-page button (on every post/page edit screen) therefore triggers a full rebuild + redeploy on Netlify, not just an update of that one page.', 'elbutschitos-html-publisher'); ?>
                    </p>
                </div>
                ...
                <div id="content2html-target-sftp" class="content2html-target-section">
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            <th><label for="sftp_username"><?php esc_html_e('Username', 'elbutschitos-html-publisher'); ?></label></th>
                        ...
                        <tr>
                            <th><?php esc_html_e('Authentication', 'elbutschitos-html-publisher'); ?></th>
                            <td>
                                <label><input type="radio" name="sftp_auth_method" value="password" <?php checked('password', $settings['sftp_auth_method']); ?> class="content2html-sftp-auth-radio"> <?php esc_html_e('Password', 'elbutschitos-html-publisher'); ?></label>
                                &nbsp;&nbsp;
                                <label><input type="radio" name="sftp_auth_method" value="key" <?php checked('key', $settings['sftp_auth_method']); ?> class="content2html-sftp-auth-radio"> <?php esc_html_e('Private key', 'elbutschitos-html-publisher'); ?></label>
                            </td>
                        </tr>
                        <tr class="content2html-sftp-auth-password">
                            <th><label for="sftp_password"><?php esc_html_e('Password', 'elbutschitos-html-publisher'); ?></label></th>
                            <td>
                                <input type="password" id="sftp_password" name="sftp_password" value="" class="regular-text" autocomplete="new-password">
                                <p class="description"><?php echo !empty($settings['sftp_password']) ? esc_html__('Already saved. Leave empty to keep it.', 'elbutschitos-html-publisher') : ''; ?></p>
                            </td>
                        </tr>
                        <tr class="content2html-sftp-auth-key">
                            <th><label for="sftp_private_key"><?php esc_html_e('Private key (PEM)', 'elbutschitos-html-publisher'); ?></label></th>
                            <td>
                                ...
                                <p class="description"><?php echo !empty($settings['sftp_private_key']) ? esc_html__('Already saved. Leave empty to keep it.', 'elbutschitos-html-publisher') : ''; ?></p>
                            </td>
                        </tr>
                        <tr class="content2html-sftp-auth-key">
                            <th><label for="sftp_passphrase"><?php esc_html_e('Passphrase (if any)', 'elbutschitos-html-publisher'); ?></label></th>
                            <td><input type="password" id="sftp_passphrase" name="sftp_passphrase" value="" class="regular-text" autocomplete="new-password"></td>
                        </tr>
                        <tr>
                            <th><label for="sftp_remote_base_path"><?php esc_html_e('Target directory on the server', 'elbutschitos-html-publisher'); ?></label></th>
                    ...
                    <p>
                        <button type="button" class="button" id="content2html-test-sftp-btn"><?php esc_html_e('Test connection', 'elbutschitos-html-publisher'); ?></button>
                ...
                <?php submit_button(__('Save settings', 'elbutschitos-html-publisher')); ?>
                </div>
            </form>
            ...
            <h2 class="title"><?php esc_html_e('Deployment', 'elbutschitos-html-publisher'); ?></h2>
            <p>
                <button type="button" class="button button-primary button-hero" id="content2html-deploy-all-btn">
                    <?php esc_html_e('Deploy all', 'elbutschitos-html-publisher'); ?>
                </button>
                <button type="button" class="button" id="content2html-deploy-assets-only-btn" <?php echo $assetsStatus['exists'] ? '' : 'disabled'; ?>>
                    <?php esc_html_e('Deploy assets only', 'elbutschitos-html-publisher'); ?>
                </button>
            </p>
            <p>
                <label>
                    <input type="checkbox" id="content2html-skip-assets" checked>
                    <?php esc_html_e('Upload assets when deploying', 'elbutschitos-html-publisher'); ?>
                </label>
                <?php if ($settings['target'] === 'netlify'): ?>
                    <br><span class="description"><?php esc_html_e('On Netlify, assets are always uploaded (a deploy replaces the entire content) - this option has no effect here.', 'elbutschitos-html-publisher'); ?></span>
                <?php endif; ?>
            </p>
            ...
            <h2 class="title"><?php esc_html_e('Markdown export', 'elbutschitos-html-publisher'); ?></h2>
            <p class="description">
                <?php esc_html_e('Exports all posts/pages as plain', 'elbutschitos-html-publisher'); ?> <code>.md</code> <?php esc_html_e('files with YAML front matter (title, date, slug) - meant for other systems (Hugo, Jekyll, Eleventy, Obsidian vault etc.),', 'elbutschitos-html-publisher'); ?>
                <strong><?php esc_html_e('not', 'elbutschitos-html-publisher'); ?></strong> <?php esc_html_e('as a deployable website. Runs independently of SFTP/Netlify; the result is a direct ZIP download.', 'elbutschitos-html-publisher'); ?>
            </p>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <p>
                    <label>
                        ...
                        <?php esc_html_e('Download images and rewrite references as relative paths', 'elbutschitos-html-publisher'); ?>
                    </label>
                    <br>
                    <span class="description">
                        <?php esc_html_e('Enabled: images are additionally included in the ZIP (under', 'elbutschitos-html-publisher'); ?> <code>wp-content/uploads/...</code>), <?php esc_html_e('references become relative - the export then also works without a running WordPress instance. Disabled: image links stay unchanged and keep pointing to your WordPress site.', 'elbutschitos-html-publisher'); ?>
                    </span>
                </p>
                <?php submit_button(__('Export as Markdown now', 'elbutschitos-html-publisher'), 'secondary'); ?>
            </form>
            </div>
        </div>
        <?php
